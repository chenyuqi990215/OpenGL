...
{
    for (double x=stx;x<enx;x+=0.01)
    {
        glColor3f(1,1,1);
        glBegin(GL_POLYGON);
            glVertex3f(x,calf(x)+dely,posz);
            glVertex3f(x+0.01,calf(x+0.01)+dely,posz);
            glVertex3f(x+0.01,calf(x+0.01)+prey,posz+0.01);
            glVertex3f(x,calf(x)+prey,posz+0.01);
        glEnd();
    }
}
void get_curve2(double stx,double enx,double dely,double posz)
{
    for (double x=stx;x<enx;x+=0.01)
    {
        glColor3f(1,1,1);
...
void display()
{
    ...
    for (int i=0;i<80;i++)
    {
        if (i!=0) get_curve1(x,-x,calf(y)-0.8,z);
        prey=calf(y)-0.8;
        y-=0.01;
        x+=0.01;
        z-=0.01;
    ...
    for (int i=0;i<80;i++)
    {
        if (i!=0) get_curve2(x,-x,calf(y)-0.8,z);
        prey=calf(y)-0.8;
        y-=0.01;
        x+=0.01;
        z+=0.01;
